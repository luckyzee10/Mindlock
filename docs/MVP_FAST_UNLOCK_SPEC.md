...
- Replace the existing multi-option, difficulty-based unlock system with two choices:
  ...
  - **Full day pass** — single in-app purchase priced at **$0.99 USD** (Small Developer Program, 15% Apple fee) that unlocks the device until the next midnight rollover.
...
| Unlock UX | Two tiles (free wait vs. $0.99 day pass), new countdown microinteraction for the free option, simplified StoreKit flow. |
...
### 1. Onboarding & Setup
1. **Welcome + Mission**: highlight “Stay focused. Unlock with intention. Every day pass donates to charity.”
2. **Daily Limit Picker**: unchanged (still require users to set allowed time).
3. **Charity Selection**: allow users to **skip** during onboarding (default to “choose a cause later”). Prompt again the first time they attempt a paid unlock so every Day Pass has a charity attached.
4. **Unlock Explainer Card** (new copy):
   - “Need a quick break? Wait 30 seconds to unlock 10 minutes.”
   - “Need the rest of the day? Donate $1 to your cause and get a Day Pass.”
5. **DifficultySelectionView.swift**: remove from navigation + delete dead code/strings once the plan is final.

### 2. Unlock Flow UX
1. **UnlockPromptView**
   - Show only two `UnlockOption` models (`.timedWait`, `.dayPass`).
   - Display pricing (“Free · wait 30s” / “$1 all-day pass”).
...
3. **Day Pass Path**
   - Initiate StoreKit product `mindlock.daypass`.
...
4. **Shared Charity Banner**: mention “15% of each Day Pass funds your chosen charity.”
...
2. `DailyLimitsManager` responsibilities:
   - Track current status: `.locked`, `.waitingCountdown`, `.timedUnlock(expiration)`, `.dayPass(expiration)`.
...
3. Confirm the mobile app still posts purchase receipts (no change in endpoint) but now always passes productId = `mindlock.daypass`.
...
7. **QA** – device testing for wait countdown, day pass, midnight rollover.
