...
type MonthlyReportJob = Job<{ month?: string }>;

interface CharityAggregate {
  charityId: string;
  charityName: string;
...
export async function generateMonthlyReport(
  ...
  const charityMap = new Map<string, CharityAggregate>();
  for (const donation of donations) {
    const aggregate =
      charityMap.get(donation.charityId) ?? {
  ...
  const impactByTier = new Map<
    string,
    { tier: string; reports: number; projectedDonationCents: number; points: number }
  >();
